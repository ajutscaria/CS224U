...
public class Inference extends AbstractILPInference<ExampleStructure> {
  ...
  public Inference(List<BioDatum> eventPredicted,
      List<BioDatum> entityPredicted, List<BioDatum> relationPredicted,
      List<Example> testSet, ILPSolverFactory solverFactory, boolean debug) {
    ...
    for (String key : relationPredicted.get(0).rankRelation.keySet()) {
      if (key.equals("NONE") || key.equals("SameEvent")
          || key.equals("PreviousEvent"))
        continue;
     ...
     * temp.add(whichevent); entityToEvent.put(j, temp); }
     */
    ...
    for (int i = 0; i < entityPredicted.size(); i++) {
      ...
      for (int j = 0; j < eventPredicted.size(); j++) {
        if (!entityPredicted.get(i).getSentence()
            ...
            || !children.contains(eventPredicted.get(j).eventNode))
          continue;
        else {
    ...
    // System.out.println("entityOverlapEvent size: " +
    // entityOverlapEvent.size());
    System.out.println("processToEvent size: " + processToEvent.size()
        + ", check:" + check);
    constraints = new ArrayList<ILPConstraintGenerator>();
    /*
     * constraints.add(new UniqueLabelConstraint()); constraints.add(new
     * ValidAConstraintGenerator()); constraints.add(new
     * EntityChildrenConstraintGenerator());
     */
  }
  ...
  protected void addConstraints(ILPSolver solver,
    ...
    for (ILPConstraint constraint : unique.getILPConstraints(entityinput,
        lexicon))
      this.addConstraint(solver, constraint);
    for (ILPConstraint constraint : unique.getILPConstraints(relationinput,
        lexicon))
      this.addConstraint(solver, constraint);
    ...
    BioprocessesInput entitydualevent = new BioprocessesInput("",
        entityToEvent.size(), entityLabels.length, eventToEntity, entityToEvent);
    ...
    BioprocessesInput entitychild = new BioprocessesInput("entity",
        entityChildren.size(), entityLabels.length, entityChildren);
    for (ILPConstraint constraint : children.getILPConstraints(entitychild,
        lexicon))
      this.addConstraint(solver, constraint);
    ...
    /*
     * System.out.println("Start adding same contradiction constraints"); for
     * (ILPConstraint constraint : same.getILPConstraints(relationinput,
     * lexicon)) this.addConstraint(solver, constraint);
     * System.out.println("finish adding same contradiction constraints");
     * 
     * System.out.println("Start adding prev contradiction constraints"); for
     * (ILPConstraint constraint : prev.getILPConstraints(relationinput,
     * lexicon)) this.addConstraint(solver, constraint);
  ...
  protected void addVariables(ILPSolver solver,
    ...
    for (int Id = 0; Id < relationPredicted.size(); Id++) {
      ...
      for (int labelId = 0; labelId < relationLabels.length; labelId++) {
        ...
        // System.out.println("Score of "+relationLabels[labelId]+
        // " for relation "+ Id + ": " + score);
        var = solver.addBooleanVariable(score);
  ...
  protected ExampleStructure getOutput(ILPSolver solver,
      ...
      for (int labelId = 0; labelId < entityLabels.length; labelId++) {
        ...
        if (solver.getBooleanValue(var)) {
          if (entityLabels[labelId].equals("E")) {
            if (debug)
              System.out.println(entityPredicted.get(entityId).eventNode + ": "
      ...
      for (int labelId = 0; labelId < relationLabels.length; labelId++) {
        ...
        if (solver.getBooleanValue(var)) {
          ...
          if (!relationLabels[labelId].equals("NONE")) {
            ...
            // relationPredicted.get(relationId).event1.getTreeNode().toString()
            // +", event2: "+
            // relationPredicted.get(relationId).event2.getTreeNode().toString());
            // System.out.println("True event1: "+
            // eventPredicted.get(relationPredicted.get(relationId).event1_index).eventNode.toString()
            // +", event2: "+
            // eventPredicted.get(relationPredicted.get(relationId).event2_index).eventNode.toString());
            notNone++;
            // System.out.println("\n");
          }
          relationPredicted.get(relationId).setPredictedLabel(
              relationLabels[labelId]);
          break;
        }
      }
    ...
    if (debug)
      for (int relationId = 0; relationId < relationPredicted.size(); relationId++) {
        ...
        var = lexicon.getVariable(varName);
        System.out
            .println(varName + " assigned as " + solver.getRealValue(var));
  ...
  public static IdentityHashSet<Tree> findPredictedEvents(
  ...
  public void setEvents(List<BioDatum> data) {
    ...
    for (Example ex : testSet) {
      List<EventMention> events = ex.prediction
       ...
       * todelete.add(events.get(i)); } } for(EventMention em:todelete){
       * events.remove(em); }
       */
    }
  }
      ...
      if (ex.id.equals(id)) {
        for (EventMention em : ex.prediction.get(EventMentionsAnnotation.class)) {
          if (em.getTreeNode() == d.eventNode
            ...
            /*
             * if (remove) { // remove argument List<ArgumentRelation> arguments
             * = em.getArguments(); for (int i = 0; i < arguments.size(); i++) {
             * if (arguments.get(i).mention.getTreeNode() == d.entityNode) {
             * em.removeArgument(i); System.out.println("found entity!"); break;
             * } } } else { // add argument EntityMention entity = new
             * EntityMention("", d.getSentence(), null);
             * entity.setTreeNode(d.entityNode); // entity.setProb(scoreE);
          ...
          if (em.getTreeNode() == d.event1.getTreeNode()) {
            ...
            for (ArgumentRelation rel : em.getArguments()) {
              if (rel.mention instanceof EventMention
                  && (rel.mention.getTreeNode() == d.event2.getTreeNode())) {
