...
public class OverlapConstraintGenerator extends ILPConstraintGenerator {
  ...
  public List<ILPConstraint> getILPConstraints(IInstance x,
    ...
    /*
     * BioprocessesInput input = (BioprocessesInput)x; HashMap<Integer,
     * HashSet<Integer>> entityOverlapEvent = input.map;
     * 
     * //entity -> overlapped event candidates should not be triggers for
     * (Integer parentId : entityOverlapEvent.keySet()) {
     * 
     * //System.out.println(Inference.getVariableName(parentId, Inference.E_ID,
     * type)); //System.out.println(entityChildren.get(parentId).size());
     * 
     * for(Integer childId : entityOverlapEvent.get(parentId)){ int[] var = new
     * int[2]; double[] coef = new double[2];
     * 
     * var[0] = lexicon.getVariable(Inference.getVariableName(parentId,
     * Inference.E_ID, "entity")); coef[0] = -1;
     * 
     * var[1] = lexicon.getVariable(Inference.getVariableName(childId,
     * Inference.O_ID, "event")); coef[1] = 1;
     * 
