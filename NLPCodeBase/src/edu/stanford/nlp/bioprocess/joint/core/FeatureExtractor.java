...
public class FeatureExtractor {
  ...
  static Set<String> nominalizations = Utils.getNominalizedVerbs();
  static HashMap<String, String> verbForms = Utils.getVerbForms();
  static List<String> SelectedAdvMods = Arrays.asList(new String[]{"first, then, next, after, later, subsequent, before, previously"});
  static HashMap<String, Integer> clusters = Utils.loadClustering();
  
  static Set<String> markWords = new HashSet<String>(), advmodWords = new HashSet<String>(), eventInsidePP = new HashSet<String>();
  static boolean useBaselineFeaturesOnly = true, runGlobalModel = false;
  static String model = "";
  static List<String> TemporalConnectives = Arrays.asList(new String[]{"before", "after", "since", "when", "meanwhile", "lately", 
  ...
  static List<String> diffClauseRelations = Arrays.asList(new String[]{"acomp", "advcl", "ccomp", "csubj", "infmod", "prepc", "purpcl", "xcomp"});
  ...
  public static FeatureVector getArgumentFV(Input input, int trigger, int argument){
    List<CoreMap> sentences = input.annotation.get(SentencesAnnotation.class);
    IntPair entityspan = input.getArgumentCandidateSpan(trigger, argument);
    CoreMap sentence = getContainingSentence(sentences, entityspan.getSource(), entityspan.getTarget());
  ...
  public static FeatureVector getRelationFV(Input input, int trig1, int trig2){
    ...
    List<Pair<String, String>> wordsInBetween = findWordsInBetween(input, event1, event2);
    //Number of sentences and words between two event mentions. Quantized to 'Low', 'Medium', 'High' etc.
    Pair<Integer, Integer> countsSentenceAndWord =  findNumberOfSentencesAndWordsBetween(input, event1, event2);
    ...
    if (sentenceBetweenEvents == 0) {
        ...
        List<Pair<String, String>> markRelations = extractMarkRelation(input, sentence1, event1, event2, trig1);
        ...
        List<Pair<String, String>> ppRelations = extractPPRelation(input, trig1, sentence1, event1, event2);
    ...
    if(isImmediatelyAfter) {
        ...
        List<Pair<String, String>> advModRelations = extractAdvModRelation(sentence1, sentence2, event1, event2);
  ...
  public static CoreMap getContainingSentence(List<CoreMap> sentences, int begin, int end) {
    ...
    for(CoreMap sentence:sentences) {
      if(sentence.get(BeginIndexAnnotation.class) <= begin && sentence.get(EndIndexAnnotation.class) >= end)
        return sentence;
    }
    return null;
  }
  ...
  public static Tree getEventNode(CoreMap sentence, int eventToken) {
    Tree syntacticParse = sentence.get(TreeCoreAnnotations.TreeAnnotation.class);
    Span entitySpan = new Span(eventToken, eventToken+1); //single word of event
    for(Tree node:syntacticParse.postOrderNodeList()) {
        if(node.isLeaf())
            continue;
  ...
  public static Tree getEntityNode(CoreMap sentence, IntPair entityspan) {      
    ...
    IntPair entityNoLastToken = new IntPair(entityspan.getSource(), entityspan.getTarget() - 1);
    while(entityNoLastToken.getTarget() - entityNoLastToken.getSource() != 0) {
        ...
        entityNoLastToken = new IntPair(entityspan.getSource(), entityspan.getTarget() - 1);
    }
    //LogInfo.logs("Missed second section");
    IntPair entityNoFirstToken = new IntPair(entityspan.getSource() + 1, entityspan.getTarget());
    while(entityNoFirstToken.getTarget() - entityNoFirstToken.getSource() != 0) {
        ...
        entityNoFirstToken = new IntPair(entityspan.getSource() + 1, entityspan.getTarget());
    }
        ...
        for(Tree node: sentence.get(TreeCoreAnnotations.TreeAnnotation.class)) {
            if(node.isPreTerminal()) {
                if(node == event1) {
                    ...
                    words.add(new Pair<String, String>(Utils.getText(node), node.value()));
                }
            }
        }
    }
  ...
  public static Pair<Integer, Integer> findNumberOfSentencesAndWordsBetween(Input input,
      Tree event1, Tree event2) {
    // TODO Auto-generated method stub
  ...
  private static List<Pair<String, String>> extractAdvModRelation(CoreMap sentence1, CoreMap sentence2, Tree event1, Tree event2) {
    ...
    Pair<String, String> advModRelation1 = extractAdvModRelation(graph1, indexedWord1, indexedWord2, event1Index, event2Index);
    Pair<String, String> advModRelation2 = extractAdvModRelation(graph2, indexedWord2, indexedWord1, event1Index, event2Index);
  ...
  private static Pair<String, String> extractAdvModRelation(
        SemanticGraph graph, IndexedWord indexedWordThis, IndexedWord indexedWordThat,
  ...
  private static String extractAdvModRelation(SemanticGraph graph, IndexedWord indexedWord) {
  ...
  private static List<Pair<String, String>> extractPPRelation(Input input, int trig1, CoreMap sentence1, Tree event1, Tree event2){
    ...
    Pair<String, String> ppRelation1 = extractPPRelation(input, trig1, sentence1, event1,
                                                                                event2, event1Index, event2Index);
    Pair<String, String> ppRelation2 = extractPPRelation(input, trig1, sentence1, event2,
  ...
  private static Pair<String, String> extractPPRelation(Input input, int trig1, CoreMap sentence1, Tree thisEvent, Tree thatEvent,                                                                                             
                                                                                        int event1Index, int event2Index) {
    Tree root = sentence1.get(TreeCoreAnnotations.TreeAnnotation.class);
    Tree node = thisEvent;
    //root.pennPrint();
  ...
  private  static List<Pair<String, String>> extractMarkRelation(Input input, CoreMap sentence1, Tree event1, Tree event2, int trig1){
    CoreMap sentence = sentence1;
    SemanticGraph graph = sentence.get(CollapsedCCProcessedDependenciesAnnotation.class);
    ...
    Pair<String, String> markRelation1 = extractMarkRelation(graph, input, trig1, indexedWord1, indexedWord2, event1Index, event2Index);
    Pair<String, String> markRelation2 = extractMarkRelation(graph, input, trig1, indexedWord2, indexedWord1, event1Index, event2Index);
  ...
  private static Pair<String, String> extractMarkRelation(SemanticGraph graph, Input input, int trig1, 
  ...
  private static boolean isInSameDependencyClauseAndChild(SemanticGraph graph, IndexedWord parent, IndexedWord word) {
