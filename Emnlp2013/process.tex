...
\begin{figure*}[ht]
  \begin{center}
    ...
    %\end{tabular}
  \end{center}
  \caption{\textcolor{red}{TODO:} add appropriate caption.}\label{fig:triad}
\end{figure*}
...
\item \textsc{Cotemp} denotes that events $u$ and $v$ overlap in time(e.g., the first two event mentions in Figure~\ref{fig:process}).
...
Our relation set contains the relations \textsc{Causes} and \textsc{Enables}, which are important for modeling processes and go beyond just temporal ordering. Since for each pair of triggers, we define at most one relation, the relation \textsc{Causes} and \textsc{Enables} take precedence over the \textsc{Prev} relation and implicitly captures it. The \textsc{Super} relation appears in temporal annotations such as the Timebank corpus \cite{Pustejovsky03} and in work on temporal logic \cite{Allen83}, but in practice it is not considered by many temporal ordering systems \cite{Chambers08,Yoshikawa09,Do12}. 
