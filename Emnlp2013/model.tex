...
Next, we add constraints that enforce that the graph structure induced by the auxiliary variables results in a tree rooted in an arbitrary node $1$ (The choice of root doesn't affect connectivity). For that we need for every $i \neq j$ a flow variable $\phi_{ij}$ which specifies the amount of flow on the directed edge $z_{ij}$.
